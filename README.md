...
- **编辑条目**：修改已有条目的 IP 与主机名，保留启用状态。
- **多选批量**：勾选条目/分组前的复选框进行多选，批量启用、禁用、移动或删除。
- **移动条目**：将条目移动到其他分组，便于重新组织规则。
- **打开文件 / 文件夹**：一键用记事本打开 hosts 文件，或在资源管理器中定位该文件。
- **实时刷新**：所有修改立即写回系统 hosts 文件并刷新列表，即时生效。
- **命令行操作**：内置 C++ 命令行引擎，支持脚本化执行 `show`、`enable`、`disable`、`add`、`edit`、`moveto`、`movetomany`、`remove` 等操作，GUI 亦复用它完成读写。
...
命令:
  ...
  enable <host|group>...  启用匹配的主机或整个分组并写回（可传多个）
  disable <host|group>... 禁用匹配的主机或整个分组并写回（可传多个）
...
- 顶部工具栏提供：刷新、添加、编辑、启用、禁用、移动到、删除、打开文件、打开文件夹。
- 支持多选：勾选条目前的复选框即可多选（按住 `Shift` 点击可范围连选）；勾选分组复选框会连带选中/取消组内全部条目。工具栏的启用/禁用/移动到/删除作用于全部选中项。
- 编辑：工具栏「编辑」或条目右键「编辑…」可修改该条目的 IP / 主机名。
- 「打开文件」用记事本打开当前 hosts；「打开文件夹」在资源管理器中定位该文件（文件不存在时打开其所在目录）。
- 复选框仅表示**选择状态**（勾选=已选中），点击不会改变启用状态；列表不显示系统默认的选中蓝底。
- 启用/禁用通过工具栏按钮或右键菜单完成；被禁用的条目以**删除线 + 半透明**显示，整组禁用时分组名也显示删除线。
- 分组标题前的复选框用于选择/取消选择整组。
- 分组右键菜单：启用分组、禁用分组、添加条目…；条目右键菜单：编辑…、启用、禁用、移动到分组…、删除。
...
- GUI 支持**编辑已有条目**（修改 IP 与主机名，保留启用状态）。
- 复选框表示选择状态，支持多选；`Shift` 点击可范围连选。
- 分组复选框可整组勾选/取消（连带选中或取消组内全部条目）。
- 工具栏批量操作：启用、禁用、移动到、删除作用于全部选中项。
- 工具栏「打开文件」（记事本）与「打开文件夹」（资源管理器定位）。
- 命令引擎新增 `edit`、`movetomany`；`enable` / `disable` / `remove` 支持多目标。

**变更**

- 启用/禁用状态改由**删除线 + 半透明**表达，复选框不再控制启用状态。
- 去掉 TreeView 自带选中蓝底，选择状态仅由复选框体现。
- 整组禁用时分组名显示删除线。

**修复**

- 快速勾选复选框触发写回导致的卡顿（勾选改为纯界面操作）。
- 展开/收起箭头失效的问题。
- 批量命令循环误用 `args[0]` 仅处理首个目标的问题。

### V1.0 - 2026-09-02

- C++ 命令行引擎：分组解析与 `show` / `apply` / `enable` / `disable` / `add` / `moveto` / `remove`。
- WPF 图形界面（MVVM，跟随系统 Fluent 主题）。
- 自解压单文件启动器；`-i` / `/i` 就地释放。
- `--file` 指定路径、`--cache` 可选 bin 缓存。
- 处理 UTF-8 BOM 与分组解析问题。
